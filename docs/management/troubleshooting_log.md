...
## 2026-02-16: BOOK_TOC_EXTRACTION_PROMPT での KeyError

### 現象
- `extract_toc` 実行時に `KeyError: '"title"'` が発生してプログラムが停止した。

### 原因
- `prompts.json` 内の JSON 形式の例示に含まれる波括弧 `{ }` が、Python の `str.format()` によってプレースホルダーと誤認された。

### 対策
- `prompts.json` 内の該当する波括弧を二重化 (`{{ }}`) してエスケープした。

## 2026-02-16: 書籍モードでの目次誤検出（Self-matching in TOC area）

### 現象
- 冒頭の目次ページにある見出しが章の開始位置として設定され、章ごとの分割が機能せず、最後の項目（Index等）に全ての本文が含まれてしまう。

### 原因
- `raw_text.find(title)` がテキスト全体の先頭から検索するため、本文ではなく目次ページ内の文字列にヒットしてしまう。

### 対策
- `toc_area_estimate` (15,000文字程度) を導入し、目次エリアをスキップして検索を開始するロジックを `split_by_toc` に追加。

## 2026-02-16: JSON 書き出し時のエスケープ漏れと不正フォーマット

### 現象
- `shared/prompts.json` をプログラムから（`write_to_file` 等で）上書きした際、リテラルな改行が含まれてしまい JSON として無効になった。
- また、`BOOK_TOC_EXTRACTION_PROMPT` 内の JSON 例示にある波括弧を `{{ }}` にエスケープし忘れたため、`.format()` 実行時に `KeyError` が発生した。

### 原因
- 手動での JSON 文字列構成や、Python リテラルをそのままファイルに書き出したことによる形式不備。

### 対策
- `replace_file_content` を用いて、既存の有効な構造を保ちつつ、必要な箇所のみを一つずつ慎重に修正した。
- 例示の波括弧は `{{` `}}` に統一。

## 2026-02-16: 書籍モードからのイントロダクション生成の削除

### 現象
- ユーザー要望により、章ごとのイントロダクション生成を廃止。

### 対策
- `shared/prompts.json` から `BOOK_CHAPTER_INTRO_PROMPT` を削除。
- `src/main.py` および `src/skills.py`、`src/constants.py` から関連するメソッド、呼び出し、定数定義をすべて削除した。

## 2026-02-17: ターミナル出力の視認性改善

### 現象
- 処理開始時に毎回「[論文モード] を開始します」と表示され、書籍モード廃止後は冗長になっていた。
- 並列翻訳フェーズ（Phase 3）で、チャンクごとの進捗が表示されるたびに改行が発生し、ログが縦に長く伸びてしまい視認性が悪かった。

### 原因
- 旧書籍モードの名残による明示的なモード表示。
- `print` 関数がデフォルトで改行を伴うため、上書き（`\r`）が正しく連動していなかった。

### 対策
- `main.py` 内の開始メッセージを「処理を開始します...」に統一。
- `print_progress` 関数を改善し、非パーセンテージ表示時も `\r` を使用して行頭から上書き表示するように変更。また、古いメッセージが残らないようスペースでパディング（`<80`）する処理を追加。

## 2026-02-17: 書籍モードでの翻訳・構造化テキストの途切れ（8kトークン制限）

### 現象
- 「Introduction: thinking through things」や「Taonga Maori」などの章で、翻訳結果が本来の長さより大幅に短くなる（並列翻訳のチャンク数が異常に少ない）。
- ログを確認すると、Introduction（約9.6万文字）に対して翻訳チャンクが 2 個しか生成されていなかった。

### 原因
1.  **構造化（Phase 2）での出力制限**: `structure_text_with_hint` がデフォルトでチャンク分割無効（`enable_chunking=False`）になっていた。英語 9.6 万文字（約2.4万トークン）を一度に構造化しようとしたため、Gemini 3 Flash の出力制限（約8,000トークン）に抵触し、構造化された Markdown 自体が冒頭部分で削られていた。
2.  **翻訳チャンクサイズ（Phase 3）の過大**: `MAX_TRANSLATION_CHUNK_SIZE` が 40,000 文字（英語）に設定されていた。これは約1万トークンに相当し、日本語に翻訳すると 1.5 万トークン程度になる可能性があるため、翻訳フェーズでも出力制限による途切れが発生しうる状態だった。

### 対策
1.  **構造化のデフォルト分割化**: `skills.py` の `structure_text_with_hint` のデフォルト引数を `enable_chunking=True` に変更。長文の章も確実に全文が構造化されるようにした。
2.  **翻訳チャンクサイズの適正化**: `shared/prompts.json` の `MAX_TRANSLATION_CHUNK_SIZE` を `16,000` 文字に縮小。翻訳後の日本語が 8,000 トークンの安全圏に収まるように設計。
3.  **検証**: テスト実行により、Introduction が 17 パートに分割されて構造化されることを確認。

## 2026-02-18: 書籍モード：翻訳の要約化・途切れと章の欠落 (v2.4)

### 現象
- **翻訳の途切れ/要約化**: 「Swaziland」章などの長大な章で、翻訳結果が短くなったり、AIが勝手に要約して「以上の内容でした」といったメタ発言で締めくくる現象が発生。
- **章の欠落**: 「Taonga Maori」章が最終出力から完全に消え、前の章に吸収されてしまう。

### 原因
1.  **翻訳上限への抵触**: `MAX_TRANSLATION_CHUNK_SIZE` が 40,000 文字（英語）だった。構造化（英→英）では問題ないが、翻訳（英→日）では文字数が膨張し、Gemini 3 Flash の出力上限（約 65,536 トークン）に抵触、AIが無理やり完結させるために要約や打ち切りを行っていた。
2.  **章分割ロジックの脆弱性**: `split_by_anchors` でアンカー（本文冒頭）が見つからない場合、その章を `continue`（スキップ）していた。これにより、OCRミス等でアンカーが不一致になると章ごと消滅していた。
3.  **非効率な検索**: `fuzzy_find` が全テキストに対してスライディングウィンドウを回しており、処理時間が長かった。

### 対策
- **チャンク設定の分離**:
    - `MAX_STRUCTURING_CHUNK_SIZE` (40,000) を新設し、構造化は広い文脈で実行。
    - `MAX_TRANSLATION_CHUNK_SIZE` を **15,000** 文字に縮小。日本語訳の膨張分を含めても安全に出力しきれるサイズに調整。
- **分割ロジックの刷新**:
    - アンカー未検出時の **スキップを廃止**。
    - **タイトル検索フォールバック**: アンカーで見つからない場合、章タイトルで場所を特定する。
    - **位置推測分割**: それでも位置が不明な章は、前後の章の境界から推測して「空」にせず内容を保持する。
- **検索の最適化**:
    - `fuzzy_find` を改善。アンカーの最初の10文字（seed）を `str.find` で高速に探し、その周辺のみ Levenshtein 距離を計算する方式に変更。
- **プロンプトの強化**:
    - `BOOK_TOC_MAPPING_PROMPT` で、アンカーとして「最初のパラグラフ（100-200文字）」を抽出するよう指示。マッチングの精度を向上。

### 検証
- `pytest tests/test_book_quality.py` により、アンカーが見つからない章も正しく処理されることを確認。
- 統合テストにより、長大な章（Swaziland）が適切なチャンク数に分割され、最後まで翻訳されることを確認。

## 2026-02-19: 翻訳出力へのレジュメ（要約）の重複混入

### 現象
- 最終出力ファイル (`_output.txt`) において、プログラムが挿入したレジュメの直後に、翻訳結果の一部としてAIが再生成したレジュメが重複して出現する。

### 原因
- 翻訳フェーズ（Phase 3）のプロンプトにおいて、精度向上のためのコンテキストとして `[Context: Summary of the paper]` を渡している。
- 強力なAIモデル（Gemini 3 Flash等）が、このコンテキスト情報も「ユーザーが翻訳結果に含めてほしい重要な情報」であると解釈し、翻訳本文の冒頭に差し込んでしまう。

### 対策
- `shared/prompts.json` の `TRANSLATION_PROMPT` を更新。
- **「コンテキスト（[Context]）は参考資料であり、出力（翻訳結果）には絶対に含めないこと」**という明示的な制約を追加し、ハルシネーション（親切心による混入）を抑制した。

## 2026-02-19: 翻訳フェーズでの進捗表示の不足

### 現象
- 並列翻訳中、コンソールには「Phase 3: 文脈を考慮した並列翻訳を実施中...」といった大まかな表示しか出ず、特に長文論文で「ハングアップしているのか、順調に処理中なのか」が判別しにくい。

### 原因
- `translate_academic` 内での並列タスク完了時に、親のプログレスコールバックが呼び出されず、詳細なステータス（完了数/全数）がユーザーに報告されていなかった。

### 対策
- `src/skills.py` の `translate_academic` を改修。
- 各翻訳チャンクの開始時（「チャンク n/total 翻訳中...」）と完了時（「チャンク n/total 完了」）にプログレスコールバックを実行するように変更。
- `main.py` の `print_progress` と連携し、同一行での詳細な進捗更新を実現。
