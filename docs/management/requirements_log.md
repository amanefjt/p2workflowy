...
## 2026-02-20: 並列処理の安定化 (v2.10)
**Goal**: 並列実行数を以前の安定した値に戻し、リクエストの成功率を優先する。
**Changes**:
- **並列リクエスト数の調整**:
    - Python版 (`src/skills.py`) および ウェブ版 (`web/src/App.tsx`) の並列実行数（Semaphore/Concurrency）を **4 から 3** に戻した。これにより、APIのレート制限や安定性をより重視する設定となった。
