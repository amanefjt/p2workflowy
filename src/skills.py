    ...
    async def translate_academic(self, clean_markdown: str, glossary_text: str = "", summary_context: str = "", progress_callback=None) -> str:
        ...
        async def call_with_logging(i, prompt_text):
            async with semaphore:
                print(f"    [Chunk {i+1}/{len(prompts)}] Starting translation...")
                try:
                    # 個別のチャンクのリトライ状況を表示するために callback を作成
                    chunk_callback = lambda msg: print(f"    [Chunk {i+1}/{len(prompts)}] {msg}")
                    res = await asyncio.to_thread(self.llm.call_api, prompt_text, chunk_callback)
                    print(f"    [Chunk {i+1}/{len(prompts)}] Finished.")
                    return str(res)
                except Exception as e:
                    print(f"    [Chunk {i+1}/{len(prompts)}] FAILED: {e}")
