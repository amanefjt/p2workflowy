...
| 用途 | 推奨モデル | 理由 | 備考 |
| :--- | :--- | :--- | :--- |
| **Web UI (既定)** | `gemini-3.1-flash-lite-preview` | **500 RPD** (Free tier) / **超低コスト** | `Thinking: High` 必須 |
| **CLI / 高品質** | `gemini-3-flash-preview` | 応答が極めて安定。1チャンクあたりの推論が重厚。 | 有料版推奨（500 RPM） |
...
## 7. API ティア管理と動時最適化 (V2.1)

2026-03-12に、APIキーのティア（有料/無料）を自動判別し、実行パラメータを動的に変更する「オートダウンシフト」機能を実装しました。

- **自動ダウンシフトロジック**:
  - 有料版設定（12並列, RPM 500）で開始。
  - `429 RESOURCE_EXHAUSTED` を検知すると、即座に無料版の安全設定（2並列, RPM 15）にブレーキをかけ、処理を中断せずに完走させます。
- **環境別デフォルト**:
  - **CLI**: `Paid`（有料版）を標準。高速性を優先。
  - **Web**: `Free`（無料版）を標準。安定性を優先。

## 8. 運用コストの目安 (Paid Tier / Gemini 3.1)

Gemini 3.1 シリーズの Paid プライシング（100万トークンあたり）に基づく試算：

| モデル | 文字入力 (Input) | テキスト出力 (Output) |
| :--- | :--- | :--- |
| `Gemini 3.1 Flash` | $0.50 | $3.00 |
| `Gemini 3.1 Flash-Lite` | **$0.075** | **$1.50** |

### ケーススタディ: `ALpdf.pdf` (約6.6万文字 / 1.3万ワード) の翻訳コスト

| 項目 | `3.1 Flash-Lite` (試算) | `3.1 Flash` (試算) | 備考 |
| :--- | :--- | :--- | :--- |
| 入力トークン (約16万tk) | ~$0.012 (1.8円) | ~$0.08 (12円) | スライディングウィンドウを含む |
| 出力トークン (約2万tk) | ~$0.030 (4.5円) | ~$0.06 (9円) | 翻訳後の全日本語テキスト |
| **合計コスト** | **約 6.3 円** | **約 21 円** | $1=150円、Ph4換算 |

> [!NOTE]
> スライディングウィンドウ（前後の文脈保持）により、純粋な本文文字数よりも入力トークンが多くカウントされますが、Lite モデルを使用すれば **論文1本を10円以下** で、最高品質（Thinking: High）の翻訳が可能です。
