...
> **【後日談・2026-07-13】定義レイヤーは撤去。** 実装・マージ後、ユーザーの疑問を受けて
> 「定義あり vs なし」を他条件固定で A/B 再検証（NST・AL の n=2）。**定義注入に翻訳品質上の
> 効果は確認できず**（armA が勝った語ゼロ、揺れた語はすべて armB が一貫、AL では脚注欠落・
> 用語集の過剰適用という副作用も観測）。機序＝en→ja 訳語対応だけで一貫性はほぼ完結し、定義文は
> inert かつレジュメと冗長。→ **定義（definition）レイヤーを撤去し、用語レイヤーは en→ja 訳語
> 対応のみに戻した**（下記本文の「定義」関連は歴史的記録）。en→ja 対応・ハイブリッドモデル既定は
> 維持。詳細は `requirements_log.md` 2026-07-13、branch `revert-term-definition-layer`。

**ステータス**: 実装・マージ済み → **定義レイヤーは 2026-07-13 に A/B 再検証で撤去**（en→ja 対応のみ存続）
**起案日**: 2026-07-11
