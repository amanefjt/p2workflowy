...
def build_tree(
    ...
    if is_book and "chapters" in anchors:
        ...
        for ch in chapters:
            ...
            role = "h1" if is_part else "h2"

            chapter_node = TreeNode(
                id=str(start_page * 10000),
                text=title,
                role=role,
                seq_index=float(start_page),
                children=[],
            )

            # 直下の段落をすべてフラットに追加 (節分割は Phase 4 で行う)
            section_chunks = []
            for i, para in enumerate(paragraphs):
                if not para.strip():
                    continue
                node_id = str(start_page * 10000 + i + 1)
                chapter_node.children.append(
                    TreeNode(
                        id=node_id,
                        text=para,
                        role="p",
                        seq_index=float(start_page) + i * 0.0001,
                    )
                )
                section_chunks.append({"text": para, "id": node_id})

            # キーを "ID|タイトル" にして一意性を保証 (Phase 4 紐付け用)
            section_key = f"{chapter_node.id}|{title}"
...
def run_phase3(
    ...
    if is_book and input_path:
        ...
        # pymupdf で章境界を検出 (ハードコードを解除し body_start_page を渡す)
        chapters = extract_book_chapters(input_path, body_start_page=body_start_page, toc=toc)

        # TOC タイトルで補正
        if toc:
            chapters = apply_toc_titles(chapters, toc, page_offset)

        # ノイズ章フィルタ:
        # TOC補正されず（raw_titleなし）、かつ段落数が少ない章は著作権ページ等のノイズとみなし除外する。
        # 閾値 MIN_PARAGRAPHS=5 は「目次テキスト + ISBN 等が混入した空ページ」を確実に除外する値。
        MIN_PARAGRAPHS = 5
        before_count = len(chapters)
        chapters = [
            ch for ch in chapters
            if ch.get("raw_title") is not None          # TOCで正式タイトルに補正された
            or len(ch.get("paragraphs", [])) >= MIN_PARAGRAPHS  # 本文が十分ある
        ]
        removed = before_count - len(chapters)
        if removed > 0:
            print_log(f"  [Phase 3] ノイズ章除去: {removed}章 (段落数 < {MIN_PARAGRAPHS} かつ TOC補正なし)")

        anchors = {"chapters": chapters}
        chunks = []
        headings = []
        exclude_keywords = []
    ...
    tree, sections_dict = build_tree(chunks, anchors, headings, exclude_keywords, is_book=is_book)
