...
- **書籍モードの章並列化**: CLI (`main.py --book`) は**有料キー使用時のみ**章を `ThreadPoolExecutor` で並列処理する（無料キー使用時は常に完全直列）。並列時は全章スレッドが同一の有料キーを共有する（複数プロジェクトへの分散は2026-09-23に撤去済み、下記参照）。統合順序は完了順ではなく `[None]*N` をインデックスで埋めて本の並び順を維持する。**Web版 (`server.py`) は `key_rotator.configure()` を一切呼ばないため常に直列のまま**（挙動不変）。並列度は `--book-concurrency`（既定値4、`1` で強制直列）。詳細・設計判断は `docs/model_optimization.md` §2.5。
- **Phase 4 のセクション内バッチは意図的に直列**: `ParallelTranslator.translate_section_chunks()` は各バッチに直前までの訳文をスライディングウィンドウとして渡すため（`previous = prompt_builder_func(all_translated)`）、バッチ間に逐次依存がある。並列化すれば速くなる余地はある（大きな節1つに偏った文書ほど顕著）が、バッチ境界で訳語・文体がぶれる劣化の方を避ける判断（2026-07-26 ユーザー決定）。**安易に並列化しないこと**。理由・不採用にした代替案は `docs/model_optimization.md` §3.1。
...
- **Phase 4 並列数**: デフォルト `max_concurrent_sections=8`。直列化（=1）は実測で大幅に遅く、避けること。**これは「バッチ」ではなく「セクション」の同時実行数**なので、セクション数の少ない論文では 8 を超えて上げても効かない（AL 論文=9セクションで 16/24 は改善なし）。根拠と実測値は `docs/model_optimization.md` §2.3・§4。
- **無料枠キーは1プロジェクト1本のみ**（2026-09-23〜）: Gemini APIのレート制限・ティア（無料/有料）はキー単位ではなくGCPプロジェクト単位のため、複数プロジェクトの無料キーをプールして枠を水増しする運用はGoogle API Termsの "will not attempt to circumvent" 条項に抵触しうると判断し撤去した（背景・根拠は `docs/management/requirements_log.md` 2026-09-23エントリ）。CLI は `GEMINI_API_KEY_FREE_1`（無料）と `GEMINI_API_KEY`（有料）のうち**設定されている方を1本だけ**使う（両方設定時は有料優先、実行中の無料→有料自動フォールバックはしない）。無料枠でも `ModelRotator` による「キー×Liteモデル」の2軸ラウンドロビン（`KeyRotator.pool_keys()` ＋ `pick_lane()`）自体は維持している——これは同一プロジェクト内でモデルごとに独立した制限を使い分けているだけで、複数プロジェクトの問題を含まない。キーが1本なので、この2軸ラウンドロビンはモデル軸のみの回転に自然に縮退する。
