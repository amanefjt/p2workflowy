...
> [!WARNING]
> **上記「原因の切り分け」は誤りだった（2026-09-15訂正）**。翌日以降も同一条件で3回連続
> 再現したため実際に切り分けた結果、「Google側の一時的な高負荷」ではなく「無料ティアでは
> `gemini-3.7/3.8-flash` がこの規模のリクエストを構造的に処理できない」が正しい原因だった。
> 詳細は下記 2026-09-15 エントリと `troubleshooting_log.md` I-45。トレースバックだけでは
> 「一時的か構造的か」は判別できないため、今後同様の事象では実際に別モデル・別ティアで
> 同一プロンプトを再現テストしてから原因を断定すること。

## 2026-09-15: `DEFAULT_MODEL_RESUME` を `gemini-3.8-flash` → `gemini-3.6-flash` へ差し戻し

- **背景**: 2026-09-10 に `gemini-3.6-flash` → `gemini-3.8-flash` へ切替（本ログ同日
  エントリ）。その後 2026-09-14〜15 に同一PDF・同一フェーズ（レジューム生成）で3回連続
  `503 UNAVAILABLE` が発生。前日は「Google側の一時的な高負荷」と説明したが、ユーザーから
  「3日連続で同じ場所が失敗するのは本当にGoogle側の問題か」と再確認を求められた。
- **切り分け**: 実際に失敗したプロンプト（入力44,840字・`thinking_level="High"`・
  `max_output_tokens=32768`）をそのまま「無料キー4本 × {3.6, 3.7, 3.8}-flash」「有料キー ×
  3.8-flash」に直接投げて比較。**無料キー4本すべてで `3.7-flash`・`3.8-flash` は503、
  `3.6-flash` のみ成功。有料キーでは `3.8-flash` も成功**。無料ティアの構造的な制約と判明
  （詳細・再現手順は `troubleshooting_log.md` I-45）。
- **副次的な発見**: `KeyRotator.best_available()` はクールダウンが明けた無料キーを有料キー
  より優先して選ぶ実装のため、無料キーのクールダウン（30〜60秒）がリトライ間隔より短いと、
  5回のリトライでは無料4本を巡回するだけで有料キーへの「最終フォールバック」に実際には
  到達しない（起動時メッセージの「429/503が続いた場合の最終フォールバック」という説明と
  実装が食い違っている）。今回は対応を見送り、ユーザーに報告済み。対応するなら
  `best_available()` の優先順位ロジック自体の見直しが必要で、「無料キーが1本でも生きて
  いるなら有料キーに落ちない」という既存の設計思想（§9）とのトレードオフを検討する必要が
  ある。
- **対応**: `core/coreprompts.json` の `DEFAULT_MODEL_RESUME` を `gemini-3.6-flash` へ
  差し戻し。参照コメント（`core/book_manager.py`・`core/phase2_meta.py`）・テスト期待値
  （`tests/unit/test_coreprompts_stage2.py`）・`docs/model_optimization.md`
  （§1.1/§1.2/§1.3/§6/§7）を追従修正。ユーザーの意向を確認したうえで実施（3.8-flash維持＋
  有料フォールバック修正、の案は見送り）。
- **ググれば分かったか**: 部分的にYes。Google AI Developers Forum に `gemini-3.6/3.7-flash`
  の503頻発報告が複数あり（有料 Priority tier でも成功率0%という報告あり）、「無料ティアは
  有料ティアよりcapacityが大幅に低く容量逼迫時は無料ティアが先に弾かれる」「`HIGH` thinking
  はthinkingトークンも通常出力と同じTPM枠を消費する」という一般論は見つかった。ただし
  「`3.7/3.8-flash` の無料枠でこの規模のリクエストが具体的に毎回503になる」という記述は
  見当たらず、実測でしか確定できなかった。

## 2026-09-23: 複数GCPプロジェクトの無料キープール運用を撤去（ToS遵守）

- **背景**: ユーザーが Gemini API Rate Limits ページ（"Rate limits are applied per project,
  not per API key."）を読み、CLI（`GEMINI_API_KEY_FREE_1〜4`、それぞれ別GCPプロジェクト）・
  Web（`GEMINI_API_KEY_WEB_1〜5`）で複数プロジェクトの無料キーをプールして無料枠を実質的に
  水増ししている現行実装が、Google API Terms の "You agree to, and will not attempt to
  circumvent, such limitations documented with each API." に抵触しうるのではと指摘。BANの
  リスクを避けるため是正することが決まった。
- **調査**: Gemini API rate-limits ページ・Gemini API Additional Terms of Service・Google
  API Terms（"API Prohibitions" / "API Limitations"）を確認。レート制限が明示的にプロジェクト
  単位であること、複数アカウント作成を名指しで禁止する条文は見当たらないが「ドキュメント化
  された制限を回避しようとしない」という包括条項があることを確認。さらにユーザー提供の
  Google AI Studio 画面（スクリーンショット）から、**ティア（無料/有料）自体もプロジェクト
  単位の属性**であり、1プロジェクトに複数キーがあっても課金を紐付けた瞬間に全キーが有料
  ティアになることを確認。これにより「無料枠を使い切ってから有料へ自動フォールバックする」
  という当初案も、実装上は必然的に複数プロジェクトをまたぐ構成になると判明した。
- **決定**（ユーザー判断、リスク最小化を優先）:
  1. 同一プロジェクト内での複数Liteモデル切替（`ModelRotator`）は問題なし。維持。
  2. 複数プロジェクトの無料キープール（CLI4本・Web5本）は撤去。
  3. 無料→有料の自動フォールバックも、実装上2プロジェクトをまたぐため撤去。**1プロジェクト
     1キーのみ**に統一する（CLI: 無料/有料のうち設定されている方を1本、両方設定時は有料
     優先。Web: デフォルトはユーザー自身のキー入力、管理者パスコードで藤田の有料キー）。
  4. 書籍モードの章並列化（旧: 無料キー本数ぶんスレッドを立てて複数プロジェクトに分散）は、
     「有料キー使用時のみ、単一の有料キーを複数スレッドで共有」する設計に作り直す（無料
     キーは常に直列）。単一プロジェクトの高いレート制限を自分の支払いの範囲で並行利用する
     だけなので複数プロジェクトの問題を含まない。
- **対応**: `core/config.py`（`GEMINI_API_KEY_FREE_2〜4`・`GEMINI_API_KEY_WEB_1〜5`を削除）・
  `core/llm_client.py`（`KeyRotator.restrict_to()`/`clear_restriction()`/`is_restricted()`/
  `paid_keys()`を削除）・`main.py`（キー選択を1本のみへ簡素化）・`core/book_manager.py`
  （章並列化を`tier=="paid"`ゲート・単一キー共有方式に書き換え）・`server.py`（`WebKeyPool`
  撤去、キー未入力リクエストを400で拒否するよう追加——`_get_client()`が`api_key or
  GEMINI_API_KEY`で暗黙に藤田の有料キーへフォールバックするのを防ぐため）・Webフロントエンド
  （`web/index.html`・`web/ronbunnihongo.html`・`web/app.js`・`web/app_ronbun.js`に
  `api_key`入力欄を復元、`e618366`以前の実装が土台）・`.env.example`・`CLAUDE.md`・
  `docs/model_optimization.md`（§2.1/§2.2/§2.5/§2.6/§4/§5/§6/§7）を追従修正。
- **既知のトレードオフ**: 章並列化で全スレッドが同一の有料キーを共有するため、スレッド
  ローカルなレートリミッタ（`apply_tier_settings`）が実際の共有クォータを正しく反映しない
  （各スレッドが独立に「自分は上限まで使える」と判断しうる）。新しい共有リミッタ機構は
  作らず、既存の429/503リトライ・バックオフに委ねる設計とした。実測で問題が出れば
  `docs/model_optimization.md` §2.5 のとおり並列数を下げるか再設計する。
- **ググれば分かったか**: 部分的にYes。「レート制限はプロジェクト単位」は公式ドキュメントに
  明記されており容易に確認できた。一方「ティアもプロジェクト単位の属性で、同一プロジェクト
  内のキーは常に同じティアになる」という点は公式ドキュメントに明記されておらず、ユーザーが
  実際の管理画面を確認して初めて分かった。
