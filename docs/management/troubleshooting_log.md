...
### I-41. TPM起因の429で「無料枠Liteプールの2モデルはTPM枠を共有している」という前提が誤りで、モデルローテーションを不必要にスキップしていた（対応済み）

- **事象**: 2026-07-26、書籍モード章並列化（`docs/model_optimization.md` §10）の実走行検証中、`relationspdf.pdf` の直列ベースライン実行（`--book-concurrency 1`）で実際に429 RESOURCE_EXHAUSTEDが発生した（Phase 0 グロッサリー生成、10:27:34）。レスポンスJSONを確認したところ、`quotaId` が `GenerateContentInputTokensPerModelPerMinute-FreeTier`（＝PerModel、モデル単位で独立集計）であり、`quotaDimensions` にも対象モデル（`gemini-3.1-flash-lite`）が明示されていた。これは §9（2026-07-26 実装）が採用していた「無料枠Liteプールの2モデルはTPM枠を共有している（`gemini_models.md`§4実測で両モデルとも上限値250,000が同一だったため）」という前提と矛盾する。
- **根本原因**: §9実装時の事前調査（`~/.claude/jobs/b0473b65/tmp/research_gemma_multikey.md` 項目B）が「上限の値が2モデルとも250,000で同一」という事実を「集計カウンターも共有している」と誤って解釈していた。同じレポートの別箇所（A-2節付近）では実は「`gemini-3.1-flash-lite`と`gemini-3.5-flash-lite`は上限が一致するが使用量カウンターは独立」と正しく記述されており、レポート内で矛盾した記述が併存していた。§9はこの誤った結論の方を採用し、`call_gemini`/`call_gemini_async`のリトライループで「quotaMetricがTPM起因と判定された429ではModelRotator.advance()を試さずキー切替へ直行する」という実装にしていた。この結果、実際にはモデル単位で独立集計されているTPM枠に対して、同一プロジェクト内で完結できたはずのモデル切替（`gemini-3.1-flash-lite`→`gemini-3.5-flash-lite`）を試さず、より高コストなキー切替（無料キープールの消費）に不必要に直行していた。
...
### I-45. `DEFAULT_MODEL_RESUME` の `gemini-3.8-flash` が無料ティアでレジューム生成のリクエストを処理できず503連発（対応済み）

- **事象**: 2026-09-10 に `DEFAULT_MODEL_RESUME` を `gemini-3.6-flash` → `gemini-3.8-flash` へ切替後、2026-09-14〜15 の3回の実行すべてで、論文PDF処理のレジューム生成（Phase 2、入力44,840字≒約1万トークン、`thinking_level="High"`、`max_output_tokens=32768`）が `503 UNAVAILABLE` (`This model is currently experiencing high demand`) を返し続けた。無料キー4本すべてを順にローテーションしても解消せず、5回のリトライを使い切って失敗した。当初は「Google側の一時的な高負荷」と判断したが、翌日以降も同一条件で再現したためユーザーから疑義が出た。
- **原因の切り分け**: 実際に失敗したプロンプト（`state/<session>/logs/debug_prompt_paper_resume_unknown.txt`）をそのまま直接 API に投げて比較したところ、**無料キー4本すべてで `gemini-3.7-flash`・`gemini-3.8-flash` が503、`gemini-3.6-flash` のみ成功（89秒）。有料キーでは `gemini-3.8-flash` も成功（48秒）**。つまり Google 全体の高負荷ではなく、**無料ティアでは `gemini-3.7-flash`/`gemini-3.8-flash` がこの規模（大きめ入力＋`HIGH` thinking＋大きな出力枠）のリクエストを安定して処理できない**という構造的な制約だった。2026-09-10 の切替判断はベンチマーク・価格・Rate Limit の数値比較のみに基づき、この規模での無料ティア実データ検証を明示的にスコープ外にしていた（`docs/management/requirements_log.md` 2026-09-10 エントリ）。
  - 副次的に、Google AI Developers Forum に `gemini-3.6/3.7-flash` の503頻発報告が複数あり（有料 Priority tier でも成功率0%という報告もある）、「無料ティアは有料ティアよりcapacityが大幅に低く、Google側の容量逼迫時は無料ティアが先に弾かれる」「`HIGH` thinking はthinkingトークンも通常出力と同じTPM枠を消費する」という一般論は事前にある程度調べがついたが、「この規模のリクエストで具体的に毎回503になる」というピンポイントな情報源は見当たらず、実測が必要だった。
- **対策**: `core/coreprompts.json` の `DEFAULT_MODEL_RESUME` を `gemini-3.6-flash` へ差し戻し（`core/book_manager.py`・`core/phase2_meta.py` のコメント、`tests/unit/test_coreprompts_stage2.py` の期待値、`docs/model_optimization.md` §1.1/§1.2/§1.3/§6/§7 も追従修正）。加えて、全リトライ失敗時のエラーメッセージに原因（429/503/400の分類）と対処法を日本語で付記する `core/llm_client.py::_build_final_error_message()` を追加し、次回以降は同種の事象がユーザーにも切り分けやすいログになるようにした。
- **教訓**: 新世代モデルへの切替は、ベンチマーク・価格・Rate Limit の数値だけで決めず、**無料ティアで実際に使う入力サイズ・thinking_level・出力トークン枠での動作確認を切替前に行う**（`golden-verification` skill 等）。

### I-46. 書籍モード Phase 0（全体レジュメ・用語集生成）が無料枠のTPM上限（250,000tok/分）を単発リクエストの時点で超過し、キー・モデルをいくら切り替えても429が解消しない（対応済み）

- **事象**: 2026-09-20・2026-09-22 の2回、`Gender of the Gift.pdf`（全文抽出 1,113,493字）の書籍モード処理で、Phase 0（`BookManager._generate_global_context`）が起動直後に429 RESOURCE_EXHAUSTEDで5回リトライ後も解消せず例外終了した。2回とも約20時間離れた別実行で、エラー内容（`quotaId: GenerateContentInputTokensPerModelPerMinute-FreeTier`, `limit: 250000`）まで完全に一致していた。
- **原因の切り分け**: `RESUME_MODEL_SAFE_CHAR_LIMIT`（600,000字、I-20）は「resumeモデル（当時 `gemini-3.5-flash`）が実測735,000字前後を超えると400を返す」問題を避けるためのガードで、超過時は既定モデル（無料枠では `gemini-3.1-flash-lite` 等のLite系）へフォールバックしていた。しかしこのフォールバックは字数を削減せず、全文をそのままLiteモデルへ渡す。`docs/gemini_models.md` §4 の実測どおり、無料枠のTPM上限（1分あたり入力トークン数）はLite・Flash問わず一律250,000で、今回のリクエスト（実測279,358tok、`gemini-3.6-flash`で確認）はこの上限を単発リクエストの時点で超えていた。無料キー・モデルをどう組み合わせても同じ250,000枠に当たるため、リトライは無意味（一時的なクォータ枯渇ではなく、リクエストサイズと無料枠の構造的なミスマッチ）。
  - 副次的に、有料キーへのフォールバックが機能していないことも判明。起動時バナーには「429/503が続いた場合の最終フォールバック」とあるが、`call_gemini` の既定 `max_retries=5` は「429ごとにモデルロー→キーローを交互に試す」実装のため、無料キー4本＋有料キー1本（計5本）の並びで5回のリトライでは無料キー3本目までしか到達せず、有料キーは一度も試されなかった。
- **実測での再検証（2026-09-22）**: `RESUME_MODEL_SAFE_CHAR_LIMIT` の元になった「resumeモデルは735,000字超で400を返す」という制約は `gemini-3.5-flash` での実測（I-20）であり、現行の `DEFAULT_MODEL_RESUME`（`gemini-3.6-flash`）で同じ制約が出るかは「要再測定」のまま保留されていた。今回、有料キー＋`gemini-3.6-flash` に同じ全文（1,113,493字＝279,358tok）を単発送信したところ、**400は再現せず、146秒で正常完了**（`Prompt: 279358tk, Output: 11499tk`）。よって少なくとも `gemini-3.6-flash` は735,000字の壁を持たない、または少なくとも今回の規模までは問題ない。
- **対策**: `core/book_manager.py` にキー・字数に応じた3分岐を実装。
  1. `--model` 明示指定時: 従来どおりユーザー選択を尊重、フォールバックなし。
  2. 有料キーが設定されている場合（`key_rotator.paid_keys()`）: 字数に関わらず resume モデル（`gemini-3.6-flash`）のまま有料キーで単発送信する（`RESUME_MODEL_SAFE_CHAR_LIMIT` によるLiteフォールバックは無料キー限定に変更）。上記実測により400のリスクが無いことを確認済み。
  3. 有料キーが無い場合: 新設した `FREE_TIER_TPM_SAFE_CHAR_LIMIT = 800_000`（無料枠TPM=250,000tokに対し、実測比 約3.99字/tok・`RESUME_MODEL_SAFE_CHAR_LIMIT` と同程度の安全マージンで算出）以下ならこれまでどおり単発送信、超えるものだけ `_generate_global_context_chunked()` でチャンク分割（段落境界優先）してmap-reduce処理する（チャンクごとに部分レジュメ・部分用語集を無料枠で生成 → レジュメは最後にもう1回のLLM呼び出しで統合し直し、用語集はコード側で英語表記の重複除去）。新設プロンプトは `core/coreprompts.json` の `BOOK_SUMMARY_COMBINE_PROMPT`。
  用語集生成（従来は600,000字ガード自体が掛かっていなかった）も同じ3分岐に統一した。
- **検証**: 単体テスト481件全合格（既存挙動に影響なし）。実際に `Gender of the Gift.pdf` の全文で `_generate_global_context()` を実行し、有料キー分岐（`gemini-3.6-flash`、レジューム279,358tok・用語集279,113tok、いずれも単発で成功、レジューム24,250字・用語集26語生成）を確認。チャンク分割分岐（B-2）も合成テキストと無料キーのみの構成で1チャンク・map-reduce統合まで通ることを確認済み（実際の全文規模でのB-2実行は今回未検証＝有料キーが無い環境での大規模書籍）。
- **教訓**: 「モデルを切り替える」フォールバックは、切替先が別の制約（ここでは無料枠TPM）に引っかからないかまで検証しないと、429から400、あるいは429のまま形を変えて再発するだけになる。また、コメントに「要再測定」と書いた仮定は、実際に問題が顕在化したタイミングで放置せず再測定すること（今回は2ヶ月近く未検証のまま残っていた）。
