...
- **Phase 4 の並列数**: デフォルト `max_concurrent_sections=8`（2026-07-22改定）。直列化（=1）は実測で約 50% 遅い、というベンチマーク結果に基づくデフォルト値です。無料枠Liteプールのラウンドロビン導入後の実測で8が4より速いことを確認し4→8に変更しました（詳細: `docs/model_optimization.md` §2・§7）。
