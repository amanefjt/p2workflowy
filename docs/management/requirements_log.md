...
- **READMEの修正**:
    - GitHub上で表示が崩れていた Mermaid 図を修正（ラベルをダブルクォートで囲み、特殊文字によるパースエラーを解消）。
    - 図内の用語を最新の「レジュメ生成」に更新。
- **テストデータの秘匿化**:
    - `tests/sample_data/` を Git の管理対象から除外し、`.gitignore` に追加。これにより、ローカルでのテスト環境を維持しつつ、GitHub 上では非表示（非公開）となるように修正。
